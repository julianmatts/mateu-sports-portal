...
const FILAS = [
  ...
  // gift card: un comprobante SIN unidades. El sistema lo cuenta como venta y como ticket
  ['10-MS Diagonal 80','Mi',9,'ANA','FcB.0001-005','GIFT CARD','01-VARIOS',0,50000],
];
...
test('ninguna línea se descarta: cupón, redondeo y rubro Otros suman', () => {
  ...
  // ANA: 120.000 + 90.000 + bolsa 500 + gift card 50.000 = 260.500
  assert.equal(de('ANA').venta, 260500);
});
...
test('los tickets del local son comprobantes distintos', () => {
  ...
  // 5 comprobantes en el archivo; el compartido le cuenta a los dos vendedores
  assert.equal(suc.total.tickets, 5);
  assert.equal(de('ANA').tickets, 4);   // incluye el de la gift card, sin unidades
...
test('un comprobante sin unidades cuenta como venta y como ticket', () => {
  const {de, suc} = agregar();
  // gift cards, señas y entregas a cuenta: el criterio viejo del cierre mensual las dejaba
  // afuera por no tener unidades, y el total nunca cerraba con el sistema
  assert.equal(de('ANA').unidades, 3);          // la gift card no suma unidades
  assert.ok(suc.total.venta >= 50000);
});
