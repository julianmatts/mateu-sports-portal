...
test('sugerencia mensual = promedio de las semanas llevado al valor más cercano', () => {
  assert.equal(E.sugerirMensual(['bien', 'bien']), 'bien');            // 10
  assert.equal(E.sugerirMensual(['bien', 'regular']), 'bien');         // 7.5 (borde incluido)
  assert.equal(E.sugerirMensual(['bien', 'mal']), 'regular');          // 5
  assert.equal(E.sugerirMensual(['regular', 'mal']), 'regular');       // 2.5 (borde incluido)
  assert.equal(E.sugerirMensual(['mal', 'mal', 'regular']), 'mal');    // 1.67
  assert.equal(E.sugerirMensual(['bien', null, 'mal']), 'regular');    // ignora semanas sin dato
  assert.equal(E.sugerirMensual([null, undefined]), null);             // ninguna con dato
  assert.equal(E.sugerirMensual([]), null);
  assert.equal(E.sugerirMensual(null), null);
});
