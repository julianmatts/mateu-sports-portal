...
test('líneas crudas: solo gerencia, validación del rango, passthrough en texto y cache en R2 (no en memoria)', async () => {
  const env = envNuevo();
  const LIN = JSON.stringify({ lineas: [{ fecha: '2026-09-22', sucursal: '02', idItem: 1, talle: 'M', codigo: 'RUG858', cantidad: 1, importe: 10, cliente: null }], cursor: null });
  const viejoFetch = globalThis.fetch;
  globalThis.fetch = async (url, opt = {}) => {
    if (/\/v1\/ventas\/lineas\?/.test(String(url))) { llamadasApi.push(String(url)); return { ok: true, status: 200, text: async () => LIN, json: async () => JSON.parse(LIN) }; }
    return viejoFetch(url, opt);
  };
  try {
    // una sucursal no puede pedir líneas (traen clientes)
    assert.equal((await pedir(env, 'lineas=1&desde=2026-09-22&hasta=2026-09-22', 'kids@mateu.com.ar')).status, 403);
    // validaciones
    assert.equal((await pedir(env, 'lineas=1&desde=2026-09-22&hasta=2026-09-21', 'julian@mateu.com.ar')).status, 400);
    assert.equal((await pedir(env, 'lineas=1&desde=2026-01-01&hasta=2026-09-21', 'julian@mateu.com.ar')).status, 400);
    assert.equal((await pedir(env, 'lineas=1&desde=2026-09-22&hasta=2026-09-22&sucursal=kids', 'julian@mateu.com.ar')).status, 400);
    assert.equal((await pedir(env, 'lineas=1&desde=2026-09-22&hasta=2026-09-22&cursor=a b', 'julian@mateu.com.ar')).status, 400);
    // admin: passthrough tal cual, con sucursal y cursor
    const r = await pedir(env, 'lineas=1&desde=2026-09-22&hasta=2026-09-22&sucursal=02&cursor=MTAwMDA%3D', 'julian@mateu.com.ar');
    assert.equal(r.status, 200); assert.equal(r.cache, 'api');
    assert.equal(r.body.lineas[0].codigo, 'RUG858');
    assert.ok(/\/v1\/ventas\/lineas\?desde=2026-09-22&hasta=2026-09-22&sucursal=02&cursor=MTAwMDA%3D$/.test(llamadasApi[0]), llamadasApi[0]);
    // segundo pedido: del cache (R2), sin ir a la API
    const r2 = await pedir(env, 'lineas=1&desde=2026-09-22&hasta=2026-09-22&sucursal=02&cursor=MTAwMDA%3D', 'julian@mateu.com.ar');
    assert.equal(r2.cache, 'fresco'); assert.equal(llamadasApi.length, 1);
    assert.equal([...env.LEGAJOS.m.keys()].filter(k => /lineas/.test(k)).length, 1, 'quedó en R2');
    // una consulta sin la key
    assert.equal((await pedir({}, 'lineas=1&desde=2026-09-22&hasta=2026-09-22', 'julian@mateu.com.ar')).status, 503);
  } finally { globalThis.fetch = viejoFetch; }
});
