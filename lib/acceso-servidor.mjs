...
// otro de esta lista. Solo vale con el PIN propio ya renovado; desde CUALQUIER dispositivo (Juli,
// 23/09/2026 — hasta ese día se exigía un dispositivo donde ese mail ya hubiera entrado), salvo uno
// que gerencia bloqueó para ese mail en el panel 🔒. Copia en shared/acceso.js.
...
/* ---------- clave maestra ---------- */
// ¿El PIN es el de alguien con clave maestra que puede abrir esta cuenta? Devuelve su mail o null.
// Vale desde cualquier dispositivo (Juli, 23/09/2026); solo se rechaza el dispositivo que gerencia
// BLOQUEÓ para ese mail en el panel 🔒. El tope de intentos por cuenta + IP sigue frenando la prueba
// de PIN al azar.
async function pinMaestro(env, u, mk, pin, devId){
  const destino = String(u.email || '').toLowerCase();
  if (MAESTRAS[destino]) return null;
