...
/* ---------- token de sesión ---------- */
async function firmarToken(env, usuario, dev){
  const p = b64u(JSON.stringify({ e: String(usuario.email).toLowerCase(), r: usuario.rol, d: dev || '', x: Date.now() + TOKEN_DIAS * DIA_MS }));
  return p + '.' + b64u(await hmac(env.SESSION_SECRET, 'tok|' + p));
}
async function leerToken(env, token){
...
function registrar(env, usuario, dev, resultado){
  const d = new Date(), ym = d.getUTCFullYear() + '-' + String(d.getUTCMonth() + 1).padStart(2, '0');
  return fb(env, 'POST', 'accesos/log/' + ym, { ts: Date.now(), mail: String(usuario.email || '').toLowerCase(),
    rol: usuario.rol || '', dev: dev.id, cod: dev.cod, etq: dev.etq, r: resultado }).catch(() => {});
}
...
export async function validarPin(env, email, pin, ip){
