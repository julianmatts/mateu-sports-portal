...
test('clave maestra: con el PIN propio se entra a otra cuenta, solo desde un dispositivo conocido', async () => {
  sembrar();
  ...
  // Dispositivo donde Juli nunca entró: su PIN no abre la cuenta de la sucursal
  assert.equal((await api({ accion: 'login', email: suc, pin: '4821', dev: DEV })).status, 401);
  // Juli entra con su cuenta → el dispositivo queda anotado → ahora sí
