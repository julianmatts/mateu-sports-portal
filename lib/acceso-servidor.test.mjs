...
test('clave maestra: con el PIN propio se entra a otra cuenta desde cualquier dispositivo, salvo uno bloqueado', async () => {
  sembrar();
  const cris = 'cristian.campion@mateu.com.ar', suc = 'calle49@mateu.com.ar';
  DB.usuarios[mailKey(cris)] = { email: cris, pin: '6157', rol: 'supervisor', pinCambio: Date.now() };
  DB.usuarios[mailKey('rrhh@mateu.com.ar')] = { email: 'rrhh@mateu.com.ar', pin: '3390', rol: 'admin', pinCambio: Date.now() };
  // Dispositivo donde Juli nunca entró: su PIN igual abre la cuenta de la sucursal (Juli, 23/09/2026)
  assert.equal((await api({ accion: 'login', email: suc, pin: '4821', dev: DEV })).maestra, 'julian@mateu.com.ar');
  // Un dispositivo que gerencia bloqueó para Juli en el panel 🔒 no sirve
  DB.accesos.dispositivos = DB.accesos.dispositivos || {};
  DB.accesos.dispositivos[mailKey('julian@mateu.com.ar')] = { [DEV.id]: { estado: 'revocado' } };
  assert.equal((await api({ accion: 'login', email: suc, pin: '4821', dev: DEV })).status, 401);
  delete DB.accesos.dispositivos[mailKey('julian@mateu.com.ar')];
  // Juli entra con su cuenta → el dispositivo queda anotado como propio (informativo, para el panel)
  await api({ accion: 'login', email: 'julian@mateu.com.ar', pin: '4821', dev: DEV });
  assert.equal(DB.accesos.dispositivos[mailKey('julian@mateu.com.ar')][DEV.id].origen, 'propio');
  const r = await api({ accion: 'login', email: suc, pin: '4821', dev: DEV });
  assert.equal(r.estado, 'ok'); assert.equal(r.maestra, 'julian@mateu.com.ar'); assert.equal(r.debeCambiarPin, false); assert.ok(r.token);
  assert.equal(r.usuario.email, suc);
  assert.equal(DB.accesos.dispositivos[mailKey(suc)], undefined, 'no deja un dispositivo pendiente en la cuenta');
  const log = Object.values(DB.accesos.log[Object.keys(DB.accesos.log)[0]]);
  assert.ok(log.some(x => x.r === 'maestra' && x.por === 'julian@mateu.com.ar' && x.mail === suc));
  // La sesión abierta sigue valiendo con su token; sin token, no (el dispositivo no está aprobado para la cuenta)
  assert.equal((await api({ accion: 'estado', email: suc, dev: DEV, loginTs: Date.now(), token: r.token })).ok, true);
  assert.equal((await api({ accion: 'estado', email: suc, dev: DEV, loginTs: Date.now() })).ok, false);
  // La cortina de inactividad acepta la clave maestra; cambiar el PIN de la cuenta, no
  assert.equal((await api({ accion: 'pin-verificar', email: suc, pin: '4821', dev: DEV })).ok, true);
  assert.equal((await api({ accion: 'pin-cambiar', email: suc, pin: '4821', nuevo: '7394', dev: DEV })).status, 401);
  // Juli abre cualquier cuenta; Cristian solo las de los locales, y nadie abre la de otro con clave maestra
  assert.equal((await api({ accion: 'login', email: 'rrhh@mateu.com.ar', pin: '4821', dev: DEV })).maestra, 'julian@mateu.com.ar');
  await api({ accion: 'login', email: cris, pin: '6157', dev: DEV });
  assert.equal((await api({ accion: 'login', email: suc, pin: '6157', dev: DEV })).maestra, cris);
  assert.equal((await api({ accion: 'login', email: 'rrhh@mateu.com.ar', pin: '6157', dev: DEV }, '2.2.2.2')).status, 401);
  assert.equal((await api({ accion: 'login', email: cris, pin: '4821', dev: DEV }, '3.3.3.3')).status, 401);
});

test('clave maestra: no vale con el PIN inicial sin renovar', async () => {
  sembrar();
  DB.usuarios[mailKey('julian@mateu.com.ar')].pinCambio = null;
  await api({ accion: 'login', email: 'julian@mateu.com.ar', pin: '4821', dev: DEV });
  assert.equal((await api({ accion: 'login', email: 'calle49@mateu.com.ar', pin: '4821', dev: DEV })).status, 401);
});
