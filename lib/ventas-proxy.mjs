/* ============================================================
   ...
   Rutas (solo GET, mismo origen que el Portal, sin CORS):
     ...
     GET /api/ventas?lineas=1&desde=YYYY-MM-DD&hasta=YYYY-MM-DD[&sucursal=NN][&cursor=…]
                                              → las líneas crudas del sistema (25/09/2026: fecha, hora,
                                                sucursal NN, vendedor, comprobante, articulo, rubro,
                                                cantidad, importe, idItem, talle, codigo, codigoArticulo
                                                = código de barras, cliente, clienteCuit), 10.000 por
                                                página con `cursor`. SOLO admin / supervisor (traen
                                                clientes). Se sirven como TEXTO tal cual llegó (sin
                                                JSON.parse en el Worker: una página pesa ~2 MB) y se
                                                cachean solo en R2, no en memoria. Las usan el Reparto
                                                de Mercadería (ventas por sucursal × artículo × talle)
                                                y Regalías / Entregas EDLP (ventas por cliente).
   ...
   Espejo en Firebase: cada vez que el proxy trae de la API el detalle de una
   sucursal (ruta …/sucursal/<slug>), escribe una copia en
   recepciones-mateu/ventaEquipo/<slug>/<lunes> con por:'api' y fuente:'api'
   (de fondo, con waitUntil), SOLO si ese nodo no existe o si ya era del
   proxy: lo que cargó a mano un encargado o gerencia no se pisa. Así los
   módulos que leen ventaEquipo directo (Academia «resultados en la venta»,
   Matts, la conciliación de RRHH, el avance del objetivo mensual, la curva
   de Diagonal 80) siguen teniendo la semana aunque nadie suba el Excel.
   VENTAS_ESPEJO=0 lo apaga.
...
const FB_VENTA_EQUIPO = 'https://recepciones-mateu-default-rtdb.firebaseio.com/ventaEquipo/';
const ROLES_TODO = ['admin', 'supervisor'];
const ROLES_PROPIO = ['sucursal', 'outlet'];
const RX_ISO = /^\d{4}-\d{2}-\d{2}$/, RX_SLUG = /^[a-z0-9-]{2,30}$/;
const RX_COD_SUC = /^\d{2}$/, RX_CURSOR = /^[A-Za-z0-9=_-]{1,80}$/;
export const LINEAS_MAX_DIAS = 62;
...
export const FALLO_LINEAS_MS = 3 * 1000;
...
async function r2Leer(env, ruta){
  ...
  try { const o = await b.get(claveR2(ruta)); if (!o) return null; const e = await o.json(); return (e && e.ts && (e.data || e.texto)) ? e : null; }
  catch (e) { return null; }
}
...
/** Copia el detalle de una sucursal a ventaEquipo/<slug>/<lunes> si nadie lo cargó a mano. */
export async function espejar(env, ruta, data){
  if (!env || env.VENTAS_ESPEJO === '0') return false;
  const m = ruta.match(/^\/v1\/ventas\/semana\/(\d{4}-\d{2}-\d{2})\/sucursal\/([a-z0-9-]+)$/);
  if (!m || !data || !Array.isArray(data.vendedores)) return false;
  const t = data.total || {};
  if (!(+t.venta > 0 || +t.tickets > 0)) return false;
  const nodo = FB_VENTA_EQUIPO + encodeURIComponent(m[2]) + '/' + m[1];
  let por = null;
  try { const r = await fetch(nodo + '/por.json'); por = r.ok ? await r.json() : null; } catch (e) { return false; }
  if (por && por !== 'api') return false;
  const payload = { semana: m[1], actualizado: data.actualizado || new Date().toISOString(), por: 'api', fuente: 'api',
    vendedores: data.vendedores, total: { venta: +t.venta || 0, tickets: +t.tickets || 0, unidades: +t.unidades || 0 }, metaTienda: 0, minimoTienda: 0 };
  try { const r = await fetch(nodo + '.json', { method: 'PUT', body: JSON.stringify(payload) }); return r.ok; } catch (e) { return false; }
}

// texto:true (las líneas crudas): la respuesta se guarda como texto, sin parsear, solo en R2.
function refrescar(env, ruta, ctx, texto){
  if (VUELO.has(ruta)) return VUELO.get(ruta);
  const f = FALLOS.get(ruta);
  // las líneas crudas se piden una vez por pantalla y el navegador reintenta a los segundos si la base
  // estaba bloqueada: para ellas el fallo se recuerda solo 3 s (para las semanas, 60 s)
  if (f && Date.now() - f.ts < (texto ? FALLO_LINEAS_MS : FALLO_MS)) { const x = new Error(f.msg); x.status = f.status; x.upstream = f.upstream; x.repetido = true; return Promise.reject(x); }
  const p = (async () => {
    ...
    if (texto) {
      ...
      FALLOS.delete(ruta);
      const ent = { ts: Date.now(), texto: cuerpo };
      await r2Escribir(env, ruta, ent);      // en memoria no: son páginas de ~2 MB
      return ent;
    }
    ...
    const esp = espejar(env, ruta, data).catch(() => false);
    if (ctx && typeof ctx.waitUntil === 'function') ctx.waitUntil(esp);
    return ent;
...
export async function obtener(env, ctx, ruta, texto){
  const ahora = Date.now();
  const salida = (ent, estado) => texto ? { texto: ent.texto, estado, ts: ent.ts } : { data: ent.data, estado, ts: ent.ts };
  let ent = texto ? null : MEM.get(ruta);
  if (!ent) { ent = await r2Leer(env, ruta); if (ent && !texto) MEM.set(ruta, ent); }
  if (ent && ahora - ent.ts < FRESCO_MS) return salida(ent, 'fresco');
  const p = refrescar(env, ruta, ctx, texto);
  if (ent && ahora - ent.ts < VIEJO_MS) {
    const bg = p.catch(() => {});
    if (ctx && typeof ctx.waitUntil === 'function') ctx.waitUntil(bg);
    return salida(ent, 'viejo');
  }
  const nuevo = await p;
  return salida(nuevo, 'api');
}

/** Las líneas crudas del sistema: passthrough en texto, solo gerencia. */
async function manejarLineas(env, ctx, url, user){
  if (ROLES_TODO.indexOf(user.rol) < 0) return err('Las líneas de venta son solo para gerencia.', 403);
  const desde = url.searchParams.get('desde') || '', hasta = url.searchParams.get('hasta') || '';
  const cod = url.searchParams.get('sucursal') || '', cursor = url.searchParams.get('cursor') || '';
  if (!esFecha(desde) || !esFecha(hasta)) return err('desde y hasta tienen que ser fechas YYYY-MM-DD.', 400);
  const dias = (Date.parse(hasta + 'T00:00:00Z') - Date.parse(desde + 'T00:00:00Z')) / 86400000;
  if (dias < 0) return err('hasta es anterior a desde.', 400);
  if (dias > LINEAS_MAX_DIAS) return err('El rango no puede pasar de ' + LINEAS_MAX_DIAS + ' días.', 400);
  if (cod && !RX_COD_SUC.test(cod)) return err('La sucursal de las líneas es el código de dos dígitos del sistema.', 400);
  if (cursor && !RX_CURSOR.test(cursor)) return err('Cursor inválido.', 400);
  let ruta = '/v1/ventas/lineas?desde=' + desde + '&hasta=' + hasta;
  if (cod) ruta += '&sucursal=' + cod;
  if (cursor) ruta += '&cursor=' + encodeURIComponent(cursor);
  let res;
  try { res = await obtener(env, ctx, ruta, true); }
  catch (e) { return err(e.message || 'No se pudo consultar la API de ventas.', e.status || 502, e.upstream ? { upstream: e.upstream } : null); }
  return new Response(res.texto, { status: 200, headers: { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'private, max-age=60',
    'X-Ventas-Cache': res.estado, 'X-Ventas-Ts': new Date(res.ts).toISOString() } });
}
...
export async function manejar(request, env, ctx){
  ...
  const lineas = url.searchParams.get('lineas') === '1';
  if (!semana && !lineas) return json({ disponible: disponible(env) });
  if (!disponible(env)) return err('La API de ventas no está configurada en el servidor (falta VENTAS_API_KEY).', 503);
  if (!lineas && !esLunes(semana)) return err('La semana tiene que ser el lunes en formato YYYY-MM-DD.', 400);
  if (!lineas && slug && !RX_SLUG.test(slug)) return err('Sucursal inválida.', 400);
  ...
  if (lineas) return manejarLineas(env, ctx, url, user);
  let propio = null;
