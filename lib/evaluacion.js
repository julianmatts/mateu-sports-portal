...
(function (root, factory) {
  ...
  /* Sugerencia del valor MENSUAL de un ítem a partir de sus valores semanales.
     Promedia los puntos de las semanas con dato y lo lleva al valor más cercano
     (promedio ≥ 7.5 → 'bien' · ≥ 2.5 → 'regular' · si no → 'mal').
     Es SUGERENCIA (el supervisor confirma o corrige), nunca automático.
     Devuelve null si ninguna semana trae dato. */
  function sugerirMensual(valores) {
    if (!valores || !valores.length) return null;
    var con = valores.filter(function (v) { return Object.prototype.hasOwnProperty.call(VALOR_PUNTOS, v); });
    if (!con.length) return null;
    var avg = con.reduce(function (a, v) { return a + VALOR_PUNTOS[v]; }, 0) / con.length;
    if (avg >= 7.5) return 'bien';
