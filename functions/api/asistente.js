...
Reglas firmes:
- STOCK: solo podés hablar de stock con lo que devuelve consultar_stock, nunca de memoria ni por el catálogo (el catálogo es lo que la empresa trabajó este año, no lo que hay). La herramienta busca por CÓDIGO: si te dan un nombre («la Kantana negra»), primero encontrá el código con buscar_catalogo (con la marca alcanza) y después consultá; si hay varios colores o modelos posibles, consultá los más probables (hasta 4 códigos en una sola llamada) o preguntá cuál. Al contestar: decí sucursal por sucursal cuántas unidades y, si la herramienta trae talles, los talles con stock; aclará SIEMPRE que es el último stock que cargó cada local en el Buscador, con su fecha, y que puede haber cambiado por ventas. Las de «sin_stock_cargado» sí cargan su stock y NO tienen ese artículo: decilo así, sin vueltas. Las sucursales que figuran «sin dato» no cargan su stock en el Buscador: no digas que no tienen, decí que hay que consultarles. Si una sucursal no abre por talle, decí el total y que el talle hay que confirmarlo con el local. Nunca des precios. Todavía no tenés acceso a ventas ni al stock del sistema de gestión.
...
async function consultarStock(inp, user) {
  ...
  const articulos = codigos.map(c => {
    ...
    return { codigo: c, descripcion: desc ? desc.descripcion : undefined, con_stock: filas, sin_stock_cargado: sin, nota: filas.length ? undefined : 'Ninguna de las sucursales con dato tiene stock cargado de este código (o el código no existe en el Buscador).' };
  });
  return {
    articulos,
