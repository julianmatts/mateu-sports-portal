...
- **Puente con el escáner del sistema (`scripts/puente-escaner/`, 09/09/2026)**: un
  ...
  cae a **polling cada 2,5 s** si el navegador no tiene `EventSource` (Win7 con IE) o
  la conexión no abre; anda en los modos `puesto` **y** `sucursal` (la cuenta del
  local también sirve, no solo `NN-consulta@`). El código `PUENTE-TEST` (menú de la
  bandeja → «Probar el puente») no busca nada: muestra «Puente conectado ✔», que es
