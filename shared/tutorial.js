...
(function(){
  ...
  var TUT = {
    ...
    barrida: { icono:'🧹', nombre:'Reparto de Mercadería', pasos:[
      { ic:'🧹', t:'Qué es', d:'Dos pestañas: la «Barrida de reserva» (el cruce semanal de la reserva del Depósito Central contra las ventas por sucursal, para decidir la reposición) y el «Reparto inicial» (lo que entró, remito por remito). Las semanas guardadas se abren desde el botón «Historial», arriba a la derecha, con buscador.' },
      { ic:'⬆️', t:'Cargar', d:'Arriba de la Barrida de reserva: tres botones — ventas por sucursal, reserva del depósito y (opcional) stock por sucursal —, la semana y Guardar. Con ventas + reserva se cruza solo por ID ITEM y talle. El stock por sucursal se sube una vez por semana: queda guardado y las demás barridas de la semana lo usan solas.' },
      { ic:'✨', t:'No repartir lo nuevo', d:'Si el reporte de reserva trae los «Días u.compra» de cada talle, tildá «No repartir lo que ingresó hace menos de N días» y elegí los días: esos talles no bajan en la barrida (van por el Reparto inicial). Se ven en violeta con «Ver los talles que no hay». En la tabla, amarillo = se manda menos de lo pedido y rojo = no se manda nada.' },
      ...
      { ic:'📏', t:'Completar curva', d:'Con el stock por sucursal abierto por talle, el reparto completa la curva solo: a cada sucursal se le suman, en verde, los talles que no tiene de artículos que ya trabaja, y en los talles centrales donde tiene poco se le agranda la curva a 2 o 3 si alcanza la reserva. El vaciado de la reserva chica también va siempre. En calzado, ninguna sucursal pasa de 3 unidades por talle (ajustable). Los parámetros quedan a la vista; «Ver solo» filtra esas líneas.' },
      { ic:'🚚', t:'Reparto inicial', d:'Subís la estadística de remitos, elegís el mes y los remitos, y sale el reparto remito por remito (cada remito tiene su ubicación). A quién: la marca asignada en Asignación de Marcas, la categoría, la venta de la marca y los meses de stock. Cuánto: una curva por sucursal, los talles centrales con 2 o 3 unidades, y queda una curva en el depósito. Guardalo para que el próximo remito del mismo artículo complete a las mismas sucursales.' },
    ...
    logistica: { icono:'🚚', nombre:'Panel General · Logística', pasos:[
      ...
      { ic:'📊', t:'Envíos e ingresos', d:'Arriba, los envíos: unidades por sucursal, distribución por rubro, evolución por mes (con el mismo período del año anterior en punteado cuando está cargado), top de marcas, disciplinas y subrubros, y «Abastecimiento vs. venta»: cuántas unidades recibió cada local por cada 100 que vendió. Abajo, la recepción del depósito con el switch «Unidades / $ al costo», e «Ingresó y no salió»: lo que entró al depósito en el período y todavía no se distribuyó.' },
      { ic:'📈', t:'Variaciones y ritmo', d:'Cada KPI compara el período elegido con el anterior de la misma cantidad de meses (▲ ▼ en %) y con el mismo período del año anterior si está cargado. En el mes en curso aparece el ritmo: unidades por día hábil, la proyección del mes y cómo viene contra el mes pasado.' },
      { ic:'🗓️', t:'Meses', d:'Al abrir se muestran los últimos 3 meses (así carga rápido); en el filtro «Mes» están todos los del año y los que elijas se bajan al momento. Si falta cargar el mes pasado, arriba aparece un aviso ámbar y les llega un recordatorio por la Bandeja.' },
      { ic:'📋', t:'Detalle y Excel', d:'Al pie está el detalle artículo por artículo (buscá por código, ID ITEM o nombre; tocá un encabezado para ordenar; «Agrupar por artículo» suma las sucursales; «⧉ Copiar» deja el listado listo para pegar). «⇩ Excel» descarga el detalle filtrado, el resumen, los ingresos con costo, «Sin distribuir» y «Vs. venta».' },
  ...
  +'background:var(--marca-navy,#0B1527);color:#fff;border:2px solid var(--marca-red,#CC0000);cursor:pointer;display:flex;align-items:center;justify-content:center;'
  ...
  +'.mtu-fab:hover{background:var(--marca-red,#CC0000);transform:translateY(-2px)}'
  ...
  +'.mtu-head{background:var(--marca-navy,#0B1527);border-bottom:3px solid var(--marca-red,#CC0000);color:#fff;padding:14px 18px;display:flex;align-items:center;gap:12px}'
  ...
  +'.mtu-bt{font-family:\'Bebas Neue\',sans-serif;font-size:26px;letter-spacing:.8px;color:var(--marca-navy,#0B1527);margin-bottom:8px}'
  ...
  +'.mtu-dot.on{background:var(--marca-red,#CC0000);transform:scale(1.25)}'
  ...
  +'border-radius:10px;padding:10px 16px;cursor:pointer;transition:all .14s;border:1px solid #d7deec;background:#fff;color:var(--marca-navy,#0B1527);white-space:nowrap;flex:0 0 auto}'
  +'.mtu-btn:hover{background:#f0f3fa}'
  +'.mtu-btn[disabled]{opacity:.35;cursor:default}'
  +'.mtu-btn[disabled]:hover{background:#fff}'
  +'.mtu-btn.pri{background:var(--marca-red,#CC0000);border-color:var(--marca-red,#CC0000);color:#fff;box-shadow:0 3px 12px rgba(204,0,0,.3)}'
